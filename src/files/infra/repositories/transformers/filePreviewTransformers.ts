...
import { MyDataFilePreviewPayload } from './MyDataFilePreviewPayload'
...
export const transformMyDataFilePreviewPayloadToFilePreview = (
  filePreviewPayload: MyDataFilePreviewPayload
): FilePreview => {
  const publicationStatuses: PublicationStatus[] = []
  filePreviewPayload.publicationStatuses.forEach((element) => {
    publicationStatuses.push(element as unknown as PublicationStatus)
  ...
  return {
    ...
    ...(filePreviewPayload.variables && { variables: filePreviewPayload.variables }),
    ...(filePreviewPayload.observations && { observations: filePreviewPayload.observations }),
    userRoles: filePreviewPayload.user_roles
  }
}
