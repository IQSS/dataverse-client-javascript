...
describe('FilesRepository', () => {
  ...
  describe('getFileThumbnailClass', () => {
    const expectedThumbnailClass = FileThumbnailClass.IMAGE;
    const testGetFileThumbnailClassResponse = {
      data: {
        status: 'OK',
        data: {
          message: expectedThumbnailClass.toString(),
        },
      },
    };

    describe('by numeric id', () => {
      test('should return thumbnail class when providing id and response is successful', async () => {
        const axiosGetStub = sandbox.stub(axios, 'get').resolves(testGetFileThumbnailClassResponse);
        const expectedApiEndpoint = `${TestConstants.TEST_API_URL}/files/${testFile.id}/thumbnailClass`;

        // API Key auth
        let actual = await sut.getFileThumbnailClass(testFile.id);
        ...
        assert.match(actual, expectedThumbnailClass);

        // Session cookie auth
        ApiConfig.init(TestConstants.TEST_API_URL, DataverseApiAuthMechanism.SESSION_COOKIE);

        actual = await sut.getFileThumbnailClass(testFile.id);
      ...
      test('should return error result on error response', async () => {
        ...
        let error: ReadError = undefined;
        await sut.getFileThumbnailClass(testFile.id).catch((e) => (error = e));

        assert.calledWithExactly(
          axiosGetStub,
          `${TestConstants.TEST_API_URL}/files/${testFile.id}/thumbnailClass`,
          TestConstants.TEST_EXPECTED_AUTHENTICATED_REQUEST_CONFIG_API_KEY,
        );
        expect(error).to.be.instanceOf(Error);
      });
    ...
    describe('by persistent id', () => {
      test('should return thumbnail class when providing persistent id and response is successful', async () => {
        const axiosGetStub = sandbox.stub(axios, 'get').resolves(testGetFileThumbnailClassResponse);
        const expectedApiEndpoint = `${TestConstants.TEST_API_URL}/files/:persistentId/thumbnailClass?persistentId=${TestConstants.TEST_DUMMY_PERSISTENT_ID}`;

        // API Key auth
        let actual = await sut.getFileThumbnailClass(TestConstants.TEST_DUMMY_PERSISTENT_ID);
        ...
        actual = await sut.getFileThumbnailClass(TestConstants.TEST_DUMMY_PERSISTENT_ID);
      ...
      test('should return error result on error response', async () => {
        ...
        let error: ReadError = undefined;
        await sut.getFileThumbnailClass(TestConstants.TEST_DUMMY_PERSISTENT_ID).catch((e) => (error = e));

        assert.calledWithExactly(
          axiosGetStub,
          `${TestConstants.TEST_API_URL}/files/:persistentId/thumbnailClass?persistentId=${TestConstants.TEST_DUMMY_PERSISTENT_ID}`,
          TestConstants.TEST_EXPECTED_AUTHENTICATED_REQUEST_CONFIG_API_KEY,
        );
        expect(error).to.be.instanceOf(Error);
      });
