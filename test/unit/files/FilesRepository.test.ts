...
describe('FilesRepository', () => {
  ...
  describe('getFileCitation', () => {
    ...
    test('should return citation when response is successful', async () => {
      ...
      const expectedApiEndpoint = `${TestConstants.TEST_API_URL}/files/${testFile.id}/versions/${DatasetNotNumberedVersion.LATEST_PUBLISHED}/citation`;
    ...
    test('should return error on repository read error', async () => {
      ...
      let error: ReadError = undefined;
      await sut
        .getFileCitation(1, DatasetNotNumberedVersion.LATEST_PUBLISHED, testIncludeDeaccessioned)
        .catch((e) => (error = e));

      assert.calledWithExactly(
        axiosGetStub,
        `${TestConstants.TEST_API_URL}/files/${testFile.id}/versions/${DatasetNotNumberedVersion.LATEST_PUBLISHED}/citation`,
        TestConstants.TEST_EXPECTED_AUTHENTICATED_REQUEST_CONFIG_API_KEY_INCLUDE_DEACCESSIONED,
      );
      expect(error).to.be.instanceOf(Error);
    });
