...
describe('uploadFile', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      testDatasetIds = await createDataset.execute(TestConstants.TEST_NEW_DATASET_DTO, testCollectionAlias)
    } catch (error) {
  ...
  const createTestFileUploadDestinations = async (
    filePath: string
  ) => {
    const filesRepository = new FilesRepository()
    const destinations = await filesRepository.getFileUploadDestinations(testDatasetIds.numericId, filePath)
